...
static struct month *month_base_create(const char **possible_months,
				       const char *name, struct ged_record *rec)
{
	if (rec->value->len != 1) {
		// TODO: err
...
static void month_base_free(struct month *m)
{
	free(m->name);
	free(m->value);
	free(m);
...
static struct month *month_create(struct tag_interface interface,
...
static struct month *month_fren_create(struct tag_interface interface,
				       struct ged_record *rec)
{
...
static struct month *month_hebr_create(struct tag_interface interface,
				       struct ged_record *rec)
{
