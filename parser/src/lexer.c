...
static bool _is_digit(char c) { return '0' <= c && c <= '9'; }
...
static bool _is_alnum(char c) { return _is_digit(c) || _is_alpha(c); }
...
{
	return (_is_alnum(c) || _is_otherchar(c) || _is_space(c) ||
		_is_hash_tag(c));
}

static bool _is_car_ret(char c) { return c == '\r'; }

static bool _is_line_feed(char c) { return c == '\n'; }
...
static struct lex_token *lex_add_token(struct lex_lexer *lexer,
				       lex_token_type type, char *lexeme)
{
...
static lex_e_valid lex_validate(struct lex_lexer *lexer, lex_token_type type,
				char c)
{
	...
	case LT_POINTER: {
		...
		switch (builder->len) {
		case 0:
			return (lex_e_valid)_is_at(c);
		case 1:
			return (lex_e_valid)_is_alnum(c);
		default: {
			if (_is_anychar_noat(c))
				return LV_CONT;
			else if (!_is_at(c))
				return LV_NOT;
	...
	case LT_ESCAPE: {
		...
		case 0:
			return (lex_e_valid)_is_at(c);
		case 1:
			return (lex_e_valid)_is_hash_tag(c);
		case 2:
			return (lex_e_valid)_is_anychar_noat(c);
		default: {
	...
	case LT_DELIM:
		return _is_space(c) ? LV_DONE : LV_NOT;
	case LT_WHITESPACE:
		return (_is_space(c) || c == '\t') ? LV_DONE : LV_NOT;
	default: {
...
{
	// if eof has not been reached and lookahead is empty, we need to wait
	// for more characters before continuing
