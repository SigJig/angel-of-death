...
static struct err_message* builder_verrf(struct ged_builder* ged, const char* format, va_list args)
{
    return ehandler_verrf(ged->ehandler, "builder", format, args);
}

static struct err_message* builder_errf(struct ged_builder* ged, const char* format, ...)
{
    va_list args;
...
struct ged_record* ged_record_construct(struct ged_builder* ged, struct parser_line* line)
{
    ...
    if (strlen(level) > 1 && level[0] == '0')
    {
        ...
        builder_errf(ged, "gedcom standard disallows leading 0 on level declarations (%s)", level);
    }
