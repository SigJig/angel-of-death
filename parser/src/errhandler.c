...
static struct err_message *ehandler_add(struct err_handler *handler,
					const char *err_type,
...
void emessage_destroy(struct err_message *msg) { free(msg->message); }
				  ...
				  const char *origin, const char *format, ...)
{
	...
	struct err_message *result =
	    ehandler_verrf(handler, origin, format, args);
